...
let url2="https://cdn.freecodecamp.org/testable-projects-fcc/data/choropleth_map/counties.json"

//This fuction make a AJAX petition and is handle the result by the callback function
function makeRequest(type,url,callback,nameFile='data.json',jsonData0=''){
    ...
    http.onreadystatechange=function(){
        if(this.readyState==4 && this.status==200){
            ...
            if(jsonData0==""){
                debugLog("status","we repeat the petition")
                makeRequest("GET",url2,handleData,'data1.json',jsonData)
            }else{
                debugLog("status","the two petition is ready")
                callback(jsonData0,jsonData)
            }
        }else if(this.readyState==4 && this.status==0){
            debugLog("Status","network not avalible")
            //this is called when the network is not avalible
            if(jsonData0==""){
                debugLog("status","we send another the petition")
                fetch(nameFile).then(response=>{console.log(response) ; return response.json()} ).then(jsonResponse=>makeRequest("GET",url2,handleData,'data1.json',jsonResponse))   
            }else{
                console.log(nameFile)
                debugLog("status","the two petition is ready")
                fetch(nameFile).then(response=>{console.log(response) ; return response.json()} ).then(jsonResponse=>{callback(jsonData0,jsonResponse);})   
            }
        }else{
...
function handleData(data0,data){
    debugLog("status","entre!")
    debugLog("data0",data0)
    debugLog("data",data)
